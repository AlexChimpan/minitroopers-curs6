...
@ApplicationScoped
@Named("TIRE_SERVICE")
public class TireService implements MaintenanceTaskTypes {
     ...
     * @param additionalData -> tire posiion for the task
    ...
    public MaintenanceTask createTask(String vin, TaskType type, String notes, Map<String, Object> additionalData) {
        var tirePos = additionalData.get("tirePosition");
        TirePosition tp = null;

        if (tirePos instanceof String s) {
            tp = TirePosition.valueOf(s);
        }
        return MaintenanceTask.createTireService(vin, notes, tp);
    }
}
